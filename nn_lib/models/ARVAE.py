...
from nn_lib.models._Loss.funLoss import kl_div, MSE, CrossEntropy, AttributeBased_regu
...
class ARVAE(LightningModule):
    ...
    def compute_loss_train(self, x, D_outputs, z_ar, metrics):
        attribute = AttributeBased_regu(z_ar, metrics, self.delta)*self.gamma
        D_loss = self.compute_loss(x, D_outputs)
        D_loss["Attribute"] = attribute
        D_loss["loss"] += attribute
        return D_loss
    
    def training_step(self, train_batch, batch_idx):
        input = train_batch["input"]
        D_metrics = train_batch["metrics"]
        if "keys_cond_data" in self.cfg.model.net:
            L_metrics = torch.stack([val for key, val in D_metrics.items() if key in self.cfg.model.net.keys_cond_data], dim=1)
        else:
            L_metrics = torch.stack([val for key, val in D_metrics.items()], dim=1)

        D_outputs = self.forward(input)
        z_ar = D_outputs["z"][:,:L_metrics.shape[-1]]
        D_loss = self.compute_loss_train(input, D_outputs, z_ar, L_metrics)
