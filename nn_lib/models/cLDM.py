...
class cLDM(LightningModule):
    ...
    def forward(self, conditioning, xt, t):
        return self.model(x=xt, timesteps=t, context=conditioning.unsqueeze(2))
    ...
    def training_step(self, train_batch, batch_idx):
        conditioning = train_batch["input_cond"]
        ...
        noise_pred = self(conditioning, noisy_z_0, t)
    ...
    def generate(self, latent, shape_data):
        ...
        with torch.no_grad():
            for t in reversed(range(self.num_timesteps)):
                noise_pred = mod(xt, torch.as_tensor(t).unsqueeze(0).to(self.cfg.model.device), latent.unsqueeze(2))
