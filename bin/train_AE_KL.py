...
def main(cfg) :
    ...
    log.info("Instantiating model <AE_KL>")
    ...
    log.info("Training ...")
    trainer.fit(AE_KL_model, Datamodule_)
    ...
    prediction = trainer.predict(AE_KL_model, Datamodule_)
    for data_emb in prediction :
        D_outputs = data_emb
        for mu in D_outputs["z_mu"] :    
            emb.append(format.torch_to_numpy(mu))
        for mu, sig in zip(D_outputs["z_mu"], D_outputs["z_sigma"]) :
            output_for_ldm["z_mu"].append(format.torch_to_numpy(mu))
            output_for_ldm["z_sigma"].append(format.torch_to_numpy(sig))
    emb = np.array(emb, dtype=np.float32)
    output_for_ldm["z_mu"] = np.array(output_for_ldm["z_mu"], dtype=np.float32)
    output_for_ldm["z_sigma"] = np.array(output_for_ldm["z_sigma"], dtype=np.float32)
    ...
    with open(os.path.join(cfg.path.output_path, "output_for_ldm.pkl"), 'wb') as handle:
        pickle.dump(output_for_ldm, handle, protocol=pickle.HIGHEST_PROTOCOL)  
    log.info("done")
