...
def main(app) :
    ...
    infarct_color = np.nanmean(L_input_norm, axis=(1,2))
    colormap = {
        "Inf_myocard":infarct_color,
        "Splitting":L_split_colormap,
    }


    print(f"Instantiating model <VanillaVAE>")
    ...
    VanillaVAE_model.load_state_dict(
        D_model["state_dict"], 
    )
    ...
    def _func(clickData, cmap):
        ...
        if clickData :
            ...
            for i,data_plot in enumerate([img, recons_img, errors_img]) :
                ...
                fig.update_layout({
                    'xaxis'+str(i+1): {'anchor': 'y'+str(i+1), 'domain': [i/3, (i+1)/3], 'matches': 'x'},
                    'yaxis'+str(i+1): {'anchor': 'x'+str(i+1), 'domain': [0.0, 1.0], 'matches': 'y', 'showticklabels': False},
                })
    ...
    def _func(dim_var, cmap) :
        ...
        inv_sigma_pts = np.squeeze(VanillaVAE_model.decoder(sigma_pts))
        ...
        for i,data_plot in enumerate(inv_sigma_pts[:5]) :
            ...
            fig.add_trace(plot_, 1,i+1)
            fig.update_layout({
                'xaxis'+str(i+1): {'anchor': 'y'+str(i+1), 'domain': [i/5, (i+1)/5], 'matches': 'x'},
                'yaxis'+str(i+1): {'anchor': 'x'+str(i+1), 'domain': [0.0, 1.0], 'matches': 'y', 'showticklabels': False},
            })
        return fig
